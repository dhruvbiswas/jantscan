...
import ca.jantscan.caprocessor.ClassFieldResolver;
...
public interface AnnotationFunctions {
    ...
    Function<Class, List<Annotation>> discoverClassAnnotations = (clazz -> {
        List<Annotation> classLevelAnnotations = new ArrayList<>();
        ...
        if (cl_annotations != null && cl_annotations.length > 0) {
            ...
            for (int i = 0; i < cl_annotations.length; i++) {
                classLevelAnnotations.add(cl_annotations[i]);
            }
        }
    ...
    Function<Class, Map<String, List<Annotation>>> discoverClassAttributeAnnotations = (clazz -> {
        Map<String, List<Annotation>> classAttributeAnnotationMap = new HashMap<>();
        ...
        if (declaredFields != null && declaredFields.size() > 0) {
            for (int i = 0; i < declaredFields.size(); i++) {
                ...
                // Debug statement, replace with a logger later
                ...
                if (fieldAnnotations != null && fieldAnnotations.length > 0) {
                    for (int j = 0 ; j < fieldAnnotations.length; j++) {
                        if (classAttributeAnnotationMap.containsKey(f.getName())) {
                            ...
                            classAttributeAnnotationMap.get(f.getName()).add(fieldAnnotations[j]);
                        } else {
                            ...
                            classAttributeAnnotationMap.put(f.getName(), l);
                        }
                    }
                }
            }
    ...
    BiFunction<Class, DiscoveredAnnotations, DiscoveredAnnotations> discoverClassConstructorAnnotations = (clazz, discoveredAnnotations) -> {
    ...
    BiFunction<Class, DiscoveredAnnotations, DiscoveredAnnotations> discoverClassMethodAnnotations = (clazz, discoveredAnnotations) -> {
        // Discover methods (public and inherited)
        // For now lets skip getDeclaredMethods
        // Ideally we should merge the results of getMethods and getDeclaredMethods
